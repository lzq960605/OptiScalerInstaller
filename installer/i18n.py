...
_STRINGS_BY_LANG: dict[Lang, AppStrings] = {
    ...
    "zh": AppStrings(
        ...
        dialogs=DialogStrings(
            installer_notice_title="安装通知",
            wiki_not_configured_detail="未配置支持游戏的维基 URL。",
            wiki_open_failed_detail="无法打开支持游戏的维基。",
            close_while_installing_body="安装正在进行中。请稍候。",
            installation_completed="安装完成。",
            installation_completed_with_name_template=(
                "[RED]OptiScaler 已安装。\n"
                "要卸载它，请删除或重命名 OptiScaler 文件 ({name})。[END]"
            ),
        ...
        gpu=GpuStrings(
            ...
            dual_selection_message=(
                "检测到双 GPU。\n"
                "请选择 OptiScaler 应安装到哪个 GPU。\n"
                "安装将使用所选 GPU 的设置。\n"
                "如果游戏在另一个 GPU 上运行，可能无法正常工作。"
            ),
            vendor_unknown="未知",
        ),
        update=UpdateStrings(
        ...
        precheck=PrecheckStrings(
            no_available_dll="没有可用的 OptiScaler DLL 名称用于安装。",
            ...
            renodx_detected_template="RenoDX: {detected}",
            mod_detected_template="MOD: {detected}",
            rdr2_blocked_mod_popup_template="[RED][DOT]当检测到某些不兼容的 MOD 时，RDR2 安装无法继续。[BR][DOT]删除以下项目后重试。[BR][DOT]检测到以下不兼容的 MOD:[END][BR]{mods}",
        ),
    ),
}
...
def translate_default_precheck_error(raw_error: str, lang: Lang) -> str:
    ...
    if lang not in {"ko", "zh"} or not error_text.startswith(_NO_AVAILABLE_DLL_PREFIX):
        return error_text
